...
export function ringResult(value) {
  if (["1", "2", "3"].includes(value)) return { id: "g1", score: "5", comment: `Answered within the three-ring KPI (${value} ${value === "1" ? "ring" : "rings"}).`, evidence: `Confirmed ring count: ${value}` };
  if (value === "4plus") return { id: "g1", score: "0", comment: "Answered after more than three rings; the KPI was not met.", evidence: "Confirmed ring count: 4 or more" };
  if (value === "unknown") return { id: "g1", score: "na", comment: "Ring count could not be confirmed and requires manual review.", evidence: "Ring count: unable to confirm" };
  return null;
}
